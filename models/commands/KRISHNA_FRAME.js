...
module.exports.onLoad = async() => {
    ...
    const path = resolve(__dirname, 'KRISHNA/BABU', 'RANI1.jpg');
    if (!existsSync(dirMaterial + "BABU")) mkdirSync(dirMaterial, { recursive: true });
    if (!existsSync(path)) await downloadFile("https://i.ibb.co/XkCJRYF6/RANI1.jpg", path);
}
...
async function makeImage({ one, two }) {
    ...
    let batgiam_img = await jimp.read(__root + "/RANI1.jpg");
